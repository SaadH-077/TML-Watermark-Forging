...
the whole image regardless of quality. `S_qlt` is only a multiplier, and **we can compute LPIPS locally
but cannot compute bit-accuracy** (we hold no detector). The only detection signal is
...
| WM_3 | **unidentified** (custom chroma) | n/a | color-axis chroma transplant (fallback) | 0.023 / ≈0.43 |
...
| n/a | 2026-07-01 | `candidate_v18`, WM_3 `trim20`+low-freq-boost | 0.8274 | flat: boost hurt WM_3 (as it hurt the 256² analogs) |
